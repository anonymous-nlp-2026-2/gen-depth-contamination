# exp_qwen_retrain_chain: Qwen2.5-1.5B retrain-chain ecological validity experiment
# Each generation: LoRA fine-tune Qwen2.5-1.5B on depth_d texts, merge weights, generate depth_{d+1}
...
# Output: data/exp_qwen_retrain_chain/depth_{0..4}.jsonl, features.csv
#         results/exp_qwen_retrain_chain/{pairwise_auc,jsd,bootstrap_ci,permutation_test,summary}.json
#
...
def finetune_lora(model_id, train_texts, device="cuda:0"):
    """Fine-tune a fresh Qwen2.5-1.5B with LoRA on the given texts, return merged model + tokenizer."""
...
def run_analysis(data_dir, results_dir, max_depth):
    ...
    # Summary
    summary = {
        "experiment": "exp_qwen_retrain_chain",
...
def main():
    parser = argparse.ArgumentParser(description="Qwen2.5-1.5B retrain-chain ecological validity experiment")
    parser.add_argument("--data_dir", type=str,
                        default="/root/autodl-tmp/gen-depth-contamination/data/exp_qwen_retrain_chain")
    parser.add_argument("--results_dir", type=str,
                        default="/root/autodl-tmp/gen-depth-contamination/results/exp_qwen_retrain_chain")
    parser.add_argument("--log_file", type=str,
                        default="/root/autodl-tmp/gen-depth-contamination/logs/exp_qwen_retrain_chain.log")
    ...
    log.info("=" * 60)
    log.info("exp_qwen_retrain_chain: Qwen2.5-1.5B retrain-chain")
