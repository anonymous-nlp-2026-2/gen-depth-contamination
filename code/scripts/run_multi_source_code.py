"""
Code Multi-Source Detection: Vocab-Only + Unified Qwen2.5-1.5B Scorer.
Parallels arXiv multi-source experiments but on Code (CodeSearchNet Python) domain.
Generators: Qwen-7B, LLaMA-8B, Mistral-7B (all single-source K*=2).
"""
import csv
import json
import time
...
SOURCES = {
    "qwen7b": {
        "data_dir": Path("/root/autodl-tmp/gen-depth-contamination/data/exp_code_qwen7b"),
        "features_csv": Path("/root/autodl-tmp/gen-depth-contamination/data/exp_code_qwen7b/features.csv"),
        "needs_rescore": True,
    },
    "llama8b": {
        "data_dir": Path("/root/autodl-tmp/gen-depth-contamination/data/exp_code_llama8b"),
        "features_csv": Path("/root/autodl-tmp/gen-depth-contamination/data/exp_code_llama8b/features.csv"),
        "needs_rescore": True,
    },
    "mistral7b": {
        "data_dir": Path("/root/autodl-tmp/gen-depth-contamination/data/exp_code_mistral7b"),
        "features_csv": Path("/root/autodl-tmp/gen-depth-contamination/data/exp_code_mistral7b/features.csv"),
        "needs_rescore": True,
    },
}
...
THETA = 0.60
SEED = 42
OUT_DIR = Path("/root/autodl-tmp/gen-depth-contamination/results/exp_code_multi_source")
...
def run_vocab_only():
    print("\n" + "=" * 60, flush=True)
    print("  PATH A: Code Vocab-Only (6 scorer-independent features)", flush=True)
    print("=" * 60, flush=True)
...
def run_unified_scorer():
    ...
    print("\n" + "=" * 60, flush=True)
    print("  PATH B: Code Unified Qwen2.5-1.5B Scorer (15D OBD features)", flush=True)
    print("=" * 60, flush=True)
    ...
    X_with_label = np.hstack([X, gen_ids.reshape(-1, 1)])
    results_with_label = run_pairwise(X_with_label, depths, doc_ids, "unified-with-label")
    kstar_with_label = compute_kstar(results_with_label, THETA)
...
def print_summary(vocab_results, unified_results):
    print("\n" + "=" * 60, flush=True)
    print("=== Code Multi-Source Results ===", flush=True)
    print("=" * 60, flush=True)
    ...
    print("  C4 multi-source K*: 2 (unified no_label)", flush=True)
    print("  arXiv multi-source K*: 3 (unified no_label), 2 (vocab-only)", flush=True)
    print("  Code single-source K*: Qwen7B=2, LLaMA8B=2, Mistral7B=2", flush=True)
    vocab_kstar = vocab_results["no_label"]["kstar"]
    print(f"  Code multi-source vocab-only K*: {vocab_kstar}", flush=True)
    if unified_results:
        uni_kstar = unified_results["no_label"]["kstar"]
        print(f"  Code multi-source unified K*: {uni_kstar}", flush=True)
...
def main():
    ...
    interim_output = {
        "experiment": "exp_code_multi_source",
        "domain": "Code (CodeSearchNet Python)",
    ...
    full_output = {
        ...
        "cross_domain_comparison": {
            ...
            "arxiv_unified_no_label_kstar": 3,
            "arxiv_vocab_only_kstar": 2,
            "code_single_source_qwen7b_kstar": 2,
            "code_single_source_llama8b_kstar": 2,
            "code_single_source_mistral7b_kstar": 2,
        },
    }
