...
CASES = [
    {
        "name": "exp-021_512tok_1v2",
        "data_dir": "/root/autodl-tmp/gen-depth-contamination/data/exp_021_length_512",
        "depth_pair": (1, 2),
        "expected_auc": 0.6073,
        "check": "ci_lower_gt_060",
    },
    {
        "name": "exp-020_t07_2v3",
        "data_dir": "/root/autodl-tmp/gen-depth-contamination/data_exp020_pythia_t07",
        "depth_pair": (2, 3),
        "expected_auc": 0.5941,
        "check": "ci_upper_lt_060",
    },
    {
        "name": "exp-020_t12_1v2",
        "data_dir": "/root/autodl-tmp/gen-depth-contamination/data_exp020_pythia_t12",
        "depth_pair": (1, 2),
        "expected_auc": 0.6172,
        "check": "ci_lower_gt_060",
    },
...
def judge(case, ci_lower, ci_upper, auc_mean):
    if case["check"] == "ci_lower_gt_060":
        if ci_lower > 0.60:
            return "PASS"
        elif auc_mean > 0.60:
            return "BORDERLINE"
            ...
            return "FAIL"
    elif case["check"] == "ci_upper_lt_060":
        if ci_upper < 0.60:
            return "PASS"
        elif auc_mean < 0.60:
            return "BORDERLINE"
...
def main():
    ...
    for case in CASES:
        ...
        verdict = judge(case, ci_lower, ci_upper, point_auc)
        ...
        all_results[name] = {
            ...
            "expected_auc": case["expected_auc"],
    ...
    output = {
        "description": "Borderline cases bootstrap CI + permutation test",
    ...
    out_path = Path("/root/autodl-tmp/gen-depth-contamination/artifacts/borderline_bootstrap_ci.json")
    ...
    for name, r in all_results.items():
        print(f"  {name} ({r['depth_pair']}): AUC={r['auc_oof']:.4f}  95% CI=[{r['ci_lower']:.4f}, {r['ci_upper']:.4f}]  p={r['perm_p']:.6f}  -> {r['verdict']}", flush=True)
