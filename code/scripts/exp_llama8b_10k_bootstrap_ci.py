"""Bootstrap CI + permutation test for exp_llama8b_10k (1v2 pair).
1000 bootstrap iterations, 95% CI + Bonferroni 99.5% CI (10 configs, alpha=0.005, z=2.807).
"""
import csv
import json
import time
...
DATA_DIR = Path("/root/autodl-tmp/gen-depth-contamination/data/exp_llama8b_10k")
OUT_PATH = Path("/root/autodl-tmp/gen-depth-contamination/results/exp_llama8b_10k/bootstrap_ci.json")
SEED = 42
N_BOOT = 1000
N_PERM = 1000
THETA = 0.60

# Bonferroni: 10 configs, alpha=0.05/10=0.005, two-sided => z=2.807
BONFERRONI_Z = 2.807
...
def bootstrap_ci(y, oof_proba, doc_ids, n_boot, rng):
    ...
    ci_lower_bonf = boot_mean - BONFERRONI_Z * boot_std
    ci_upper_bonf = boot_mean + BONFERRONI_Z * boot_std

    return point_auc, ci_lower_95, ci_upper_95, ci_lower_bonf, ci_upper_bonf, boot_aucs
...
def main():
    ...
    for d0, d1 in pairs:
        ...
        point_auc, ci95_lo, ci95_hi, ci_bonf_lo, ci_bonf_hi, boot_aucs = bootstrap_ci(
        ...
        print(f"  Bonferroni 99.5% CI: [{ci_bonf_lo:.4f}, {ci_bonf_hi:.4f}]", flush=True)
        ...
        bonf_pass = ci_bonf_lo > THETA if point_auc > THETA else None
        print(f"  Bonferroni PASS (lower > 0.60): {bonf_pass}", flush=True)
        ...
        all_results[pair_name] = {
            ...
            "ci_bonferroni_lower": round(ci_bonf_lo, 4),
            "ci_bonferroni_upper": round(ci_bonf_hi, 4),
    ...
    output = {
        "experiment": "exp_llama8b_10k",
        "description": "Bootstrap CI (1000 iters) + Bonferroni 99.5% CI + permutation test",
        "method": "GroupKFold(5) OOF + doc-level bootstrap(1000) + label permutation(1000)",
        "bonferroni": {"n_configs": 10, "alpha": 0.005, "z": BONFERRONI_Z},
        "theta": THETA,
        "seed": SEED,
    ...
    for name, r in all_results.items():
        bonf_str = f"  Bonf PASS={r['bonferroni_pass']}" if r['bonferroni_pass'] is not None else ""
        print(f"  {name}: AUC={r['auc_oof']:.4f}  95%CI=[{r['ci_95_lower']:.4f}, {r['ci_95_upper']:.4f}]  "
              f"Bonf99.5%=[{r['ci_bonferroni_lower']:.4f}, {r['ci_bonferroni_upper']:.4f}]  "
              f"p={r['perm_p']:.6f}{bonf_str}", flush=True)
