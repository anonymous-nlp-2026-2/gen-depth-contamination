...
EXPERIMENTS = {
    "Qwen-7B": "exp_arXiv_qwen7b_crossscorer",
    "Qwen-1.5B": "exp_arXiv_qwen1b5_crossscorer",
    "LLaMA-8B": "exp_arXiv_llama8b_crossscorer",
    "Mistral-7B": "exp_arXiv_mistral7b_crossscorer",
}

DATA_ROOT = Path("/root/autodl-tmp/gen-depth-contamination/data")
OUT_PATH = Path("/root/autodl-tmp/gen-depth-contamination/artifacts/crossscorer_bootstrap_ci.json")
N_BOOT = 10000
N_PERM = 10000
MAX_DEPTH = 5
SEED = 42
...
def main():
    ...
    for model_name, exp_dir in EXPERIMENTS.items():
        t_start = time.time()
        print(f"\n{'#'*60}", flush=True)
        print(f"# {model_name} ({exp_dir})", flush=True)
        print(f"{'#'*60}", flush=True)
        
        data_dir = DATA_ROOT / exp_dir
        ...
        for k in range(1, MAX_DEPTH + 1):
            ...
            pairs_results[pair] = {
                ...
                "ci_upper_above_theta": bool(ci_upper > THETA),
        ...
        all_results[model_name] = {
            "experiment": exp_dir,
            "kstar_ci": kstar,
            "total_seconds": round(model_time, 1),
            "pairs": pairs_results,
        }
        
        print(f"\n  K* (CI-based) = {kstar}", flush=True)
        print(f"  Model total time: {model_time:.1f}s", flush=True)
    
    total_time = time.time() - t_global
    
    output = {
        "method": "GroupKFold(5) OOF + doc-level bootstrap + label permutation",
        "scorer": "Pythia-1.4B (cross-scorer)",
        "n_bootstrap": N_BOOT,
    ...
    for model_name, mr in all_results.items():
        print(f"\n{model_name} (K*={mr['kstar_ci']}):", flush=True)
        for pair, r in mr["pairs"].items():
            ci_flag = "PASS" if r["ci_lower_above_theta"] else "FAIL"
            print(f"  {pair}: AUC={r['auc']:.4f}  95%CI=[{r['ci_lower']:.4f}, {r['ci_upper']:.4f}]  p_perm={r['perm_p']:.6f}  [{ci_flag}]", flush=True)
