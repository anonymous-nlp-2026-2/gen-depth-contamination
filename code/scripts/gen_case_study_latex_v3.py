...
cases = [
    ...
    },
    {
        ...
        'desc': r'An ML abstract on multiplex graphs (depth~0) retains topical coherence at depth~1 but collapses into hallucinated acknowledgement boilerplate by depth~2. Raw \LaTeX{} commands (e.g., \verb|\cite|, \verb|\section*|) appear in the generated text, reflecting the model' + "'" + r's tendency to reproduce structural templates rather than substantive content.',
    },
]
...
def get_text(path, doc_id, max_words=50):
    with open(path) as f:
        for line in f:
            d = json.loads(line)
            if d['doc_id'] == doc_id:
...
# Verify: show lines containing backslash to check escaping
import re
for i, line in enumerate(L):
    if 'PLACEHOLDER' in line or 'BACKSLASH' in line:
        print(f"BUG on line {i}: {line[:100]}")

# Show arXiv depth 1 excerpt to verify
text1 = get_text(f"{BASE}/data/exp_024_arxiv/depth_1.jsonl", 1079, max_words=30)
escaped1 = escape_latex(text1)
print(f"\narXiv depth 1 escaped: {escaped1[:200]}")
