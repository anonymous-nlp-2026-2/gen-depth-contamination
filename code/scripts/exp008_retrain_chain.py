# exp-008: Retrain-chain ecological validity experiment
# Each generation: LoRA fine-tune Pythia-1.4B on depth_d → generate depth_{d+1}
# Independent LoRA per generation (no accumulation), simulating real-world model collapse scenario
...
SEED_DATA = "/root/autodl-tmp/gen-depth-contamination/data_exp020_pythia_greedy/depth_0.jsonl"
...
def finetune_lora(base_model_path, train_texts, device="cuda:0"):
    ...
    log.info(f"LoRA params: {trainable:,} / {total:,} ({100*trainable/total:.2f}%)")
...
# ---------------------------------------------------------------------------
# Phase 2: Text Generation (continuation mode for base model)
# ---------------------------------------------------------------------------
...
def generate_texts(model, tokenizer, input_texts, device="cuda:0"):
    ...
    for start in tqdm(range(0, len(input_texts), GEN_BATCH_SIZE), desc="Generating"):
        ...
        # continuation mode: use first half as prefix, generate rest
        prefix_texts = []
...
def run_analysis(data_dir, results_dir, max_depth):
    ...
    # Summary
    summary = {
        "experiment": "exp-008 retrain-chain",
        "model": "Pythia-1.4B",
        "scorer": "Qwen2.5-1.5B",
...
def main():
    parser = argparse.ArgumentParser(description="exp-008: retrain-chain ecological validity")
    parser.add_argument("--data_dir", type=str,
                        default="/root/autodl-tmp/gen-depth-contamination/data/exp_008_retrain_chain")
    parser.add_argument("--results_dir", type=str,
                        default="/root/autodl-tmp/gen-depth-contamination/results/exp_008_retrain_chain")
    parser.add_argument("--log_file", type=str,
                        default="/root/autodl-tmp/gen-depth-contamination/logs/exp_008_retrain_chain.log")
    ...
    log.info("=" * 60)
    log.info("exp-008: Retrain-chain ecological validity experiment")
    ...
    if not args.skip_generation:
        ...
        for d in range(args.max_depth):
            ...
            # Load training data for this depth
            ...
            # Fine-tune with LoRA
            model, tokenizer = finetune_lora(PYTHIA_PATH, train_texts, device=args.device)
