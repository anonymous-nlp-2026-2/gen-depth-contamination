...
OUT_PATH = Path("/root/autodl-tmp/gen-depth-contamination/artifacts/exp016_bootstrap_ci.json")
N_BOOT = 10000
N_PERM = 10000
MAX_DEPTH = 5
SEED = 42
...
def bootstrap_ci(y, oof_proba, doc_ids, n_boot, rng):
    ...
    # Vectorized: map each sample to a doc index
    _, inverse = np.unique(doc_ids, return_inverse=True)
    # Pre-build index arrays per doc for fast gathering
...
def permutation_test(y, oof_proba, doc_ids, n_perm, rng, observed_auc):
    ...
    for _ in range(n_perm):
        # Vectorized: random binary label per doc, expand to sample level
...
def main():
    ...
    output = {
        "experiment": "exp-016 Qwen2.5-1.5B base continuation",
