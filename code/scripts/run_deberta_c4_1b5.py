#!/usr/bin/env python3
"""
DeBERTa-v3-base on C4 1.5B data: 6-class ordinal depth classification.
5-fold GroupKFold, pairwise AUC with doc-level bootstrap CI, K* determination.
Runs a single config per invocation (--config) for multi-GPU parallelism.
"""
...
RESULTS_DIR = ROOT / "results" / "exp_deberta_c4_1b5"

EXPERIMENTS = {
    "qwen_rw": {
        "data_dir": ROOT / "data_exp016_qwen_base",
        "obd_path": ROOT / "results_exp016_qwen_base" / "pairwise_auc.json",
    },
    "pythia_greedy": {
        "data_dir": ROOT / "data_exp020_pythia_greedy",
        "obd_path": ROOT / "results_exp020_pythia_greedy" / "pairwise_auc.json",
    },
    "qwen_cont256": {
        "data_dir": ROOT / "data" / "exp_021_length_256",
        "obd_path": ROOT / "results" / "exp_021_length_256" / "pairwise_auc.json",
    },
}
...
def train_fold(tr_ids, tr_masks, tr_labels, va_ids, va_masks, va_labels, device, fold):
    ...
    va_dl = DataLoader(DS(va_ids, va_masks, va_labels), batch_size=BS*2, collate_fn=collate, num_workers=2, pin_memory=True)
...
def load_obd(obd_path):
    if obd_path.exists():
        data = json.load(open(obd_path))
...
def main(config_name):
    ...
    exp_dir = cfg["data_dir"]
    obd_path = cfg["obd_path"]

    device = torch.device("cuda:0")
    log.info(f"Config: {config_name}, Device: {device} ({torch.cuda.get_device_name(0)})")
    ...
    np.savez_compressed(RESULTS_DIR / f"{config_name}_oof.npz", probs=oof, labels=labels, doc_ids=doc_ids)

    pw, kstar = pairwise_auc_ci(labels, oof, doc_ids)
    elapsed = time.time() - t0

    log.info(f"\nPairwise AUC ({config_name}):")
    for pk, v in pw.items():
        log.info(f"  {pk}: {v['auc']:.4f} [{v['ci_lower']:.4f}, {v['ci_upper']:.4f}]")
    log.info(f"  K* (theta={THETA}): {kstar}  Time: {elapsed:.0f}s")

    obd = load_obd(obd_path)
    exp_res = {"experiment": config_name, "model": MODEL_NAME, "method": "DeBERTa-v3-base fine-tuned 6-class",
    ...
    json.dump(exp_res, open(RESULTS_DIR / f"{config_name}.json", "w"), indent=2)
    log.info(f"Saved: {RESULTS_DIR / f'{config_name}.json'}")

    if obd:
        log.info(f"\n{'='*60}\nDeBERTa vs OBD-LightGBM\n{'='*60}")
        for pk in ["0v1", "1v2", "2v3", "3v4", "4v5"]:
            da = pw[pk]["auc"]
            cl, ch = pw[pk]["ci_lower"], pw[pk]["ci_upper"]
            oa = obd.get(pk)
            if oa:
                log.info(f"{config_name:<18} {pk} DeBERTa={da:.4f} [{cl:.4f},{ch:.4f}] OBD={oa} delta={da-oa:+.4f}")
            else:
                log.info(f"{config_name:<18} {pk} DeBERTa={da:.4f} [{cl:.4f},{ch:.4f}]")
        log.info(f"  K*(DeBERTa)={kstar}")
    log.info("Done.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", required=True, choices=list(EXPERIMENTS.keys()))
