# exp_pythia_retrain_depth7: Pythia-1.4B retrain-chain depth extension (depth 5-7)
#
# Continues from depth 0-4 data produced by exp_008_retrain_chain.
# Each generation: fresh LoRA fine-tune on depth_d texts -> merge -> generate depth_{d+1}
#
# Input:  depth_0..4.jsonl from exp_008_retrain_chain (auto-skipped)
...
PYTHIA_MODEL_PATH = "EleutherAI/pythia-1.4b"
...
def run_analysis(data_dir, results_dir, max_depth):
    ...
    summary = {
        "experiment": "exp_pythia_retrain_depth7",
        "model": PYTHIA_MODEL_PATH,
        "scorer": SCORER_PATH,
        "protocol": "retrain-chain (independent LoRA per generation, merge before next)",
        "lora_config": {"rank": LORA_RANK, "alpha": LORA_ALPHA, "target_modules": "auto-detected"},
...
def main():
    parser = argparse.ArgumentParser(description="Pythia-1.4B retrain-chain depth extension (5-7)")
    parser.add_argument("--data_dir", type=str,
                        default="/root/autodl-tmp/gen-depth-contamination/data/exp_008_retrain_chain")
    parser.add_argument("--results_dir", type=str,
                        default="/root/autodl-tmp/gen-depth-contamination/results/exp_pythia_retrain_depth7")
    parser.add_argument("--log_file", type=str,
                        default="/root/autodl-tmp/gen-depth-contamination/logs/exp_pythia_retrain_depth7.log")
    ...
    log.info("=" * 60)
    log.info("exp_pythia_retrain_depth7: Pythia-1.4B depth extension (5-7)")
    log.info(f"Model: {PYTHIA_MODEL_PATH}")
    log.info(f"Scorer: {SCORER_PATH}")
    log.info(f"LoRA: rank={LORA_RANK}, alpha={LORA_ALPHA}, target=auto-detect")
    ...
    if not args.skip_generation:
        ...
        for d in range(args.max_depth):
            ...
            model, tokenizer = finetune_lora(PYTHIA_MODEL_PATH, train_texts, device=args.device)
