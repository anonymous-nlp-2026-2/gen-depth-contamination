...
DATA_DIR = Path("/root/autodl-tmp/gen-depth-contamination/data/exp_008_retrain_chain")
OUT_PATH = Path("/root/autodl-tmp/gen-depth-contamination/artifacts/exp027_retrain_bootstrap.json")
N_BOOT = 10000
N_PERM = 10000
MAX_DEPTH = 4
SEED = 42
...
def main():
    ...
    output = {
        "experiment": "exp-027 retrain-chain bootstrap CI (based on exp-008 v2)",
        "source_data": "exp_008_retrain_chain features.csv",
    ...
    print("\n" + "="*70, flush=True)
    print(f"{'pair':<6} | {'AUC_point':>10} | {'CI_lower':>10} | {'CI_upper':>10} | {'perm_p':>10} | CI_lower > theta?", flush=True)
    print("-"*70, flush=True)
    for pair, r in results.items():
        flag = "Yes" if r["ci_lower_above_theta"] else "No"
        print(f"{pair:<6} | {r['auc_point']:>10.4f} | {r['ci_lower']:>10.4f} | {r['ci_upper']:>10.4f} | {r['perm_p']:>10.6f} | {flag}", flush=True)
    print(f"\nK* (CI-based, theta={THETA}) = {kstar}", flush=True)
