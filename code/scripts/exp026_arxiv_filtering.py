#!/usr/bin/env python3
"""exp-026: arXiv Graduated vs Binary Filtering on Qwen2.5-1.5B.

Three strategies: no_filter (all depths), binary (depth 0-1 only),
graduated_a09 (weight=0.9^depth). LoRA rank=4, alpha=8.

Usage:
    python scripts/exp026_arxiv_filtering.py --strategy no_filter --seed 42 --device cuda:0
    python scripts/exp026_arxiv_filtering.py --strategy graduated_a09 --seed 42 --dry_run
"""
...
BINARY_MAX_DEPTH = 1  # binary keeps depth 0-1 only (Director: remove depth>=2)
GRADUATED_ALPHA = 0.9
MAX_DEPTH = 5
...
def prepare_data(strategy, seed):
    ...
    if strategy == "no_filter":
        ...
        for d in range(BINARY_MAX_DEPTH + 1):
            records = load_jsonl(DATA_DIR / f"depth_{d}.jsonl")
            all_texts.extend(r["text"] for r in records)
            all_weights.extend([1.0] * len(records))
...
def continued_pretrain_lora(texts, weights, output_dir, device, seed):
    ...
    log.info(f"Loading Qwen2.5-1.5B...")
...
def main():
    parser = argparse.ArgumentParser(
        description="exp-026: arXiv Graduated vs Binary Filtering")
    parser.add_argument("--strategy", type=str, required=True,
                        choices=["no_filter", "binary", "graduated_a09"])
    ...
    results_base = PROJECT_ROOT / "results" / "exp_026_arxiv_filtering"
    ...
    log_path = PROJECT_ROOT / "logs" / "exp026" / f"{args.strategy}_seed{args.seed}.log"
    ...
    log.info("=" * 60)
    log.info(f"exp-026: arXiv Graduated vs Binary Filtering")
    ...
    log.info(f"Binary keeps depth 0-{BINARY_MAX_DEPTH}, graduated alpha={GRADUATED_ALPHA}")
    log.info("=" * 60)
