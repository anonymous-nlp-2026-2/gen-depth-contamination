...
def main():
    ...
    for d in range(MAX_DEPTH + 1):
        n = (depths == d).sum()
        gens_at_d = [int((gen_ids[(depths == d) & (gen_ids == g)]).sum() / g) if g > 0 else (gen_ids[(depths == d) & (gen_ids == g)]).shape[0] for g in range(3)]
    ...
    output = {
        "experiment": "exp_multi_source_detection",
        "description": "Multi-source contamination detection: 3 generators mixed pool",
    ...
    print("=" * 60, flush=True)
    print(f"Single-source K* (reference): Qwen=2, Pythia=2, OLMo=2", flush=True)
