"""
Wiki Multi-Source Detection: Vocab-Only + Unified Qwen2.5-1.5B Scorer.
Parallels arXiv/C4 multi-source experiments but on Wiki domain.
"""
import csv
import json
import time
...
SOURCES = {
    "qwen1b5": {
        "data_dir": Path("/root/autodl-tmp/gen-depth-contamination/data/exp_023_wiki"),
        "features_csv": Path("/root/autodl-tmp/gen-depth-contamination/data/exp_023_wiki/features.csv"),
        "needs_rescore": False,
    },
    "llama8b": {
        "data_dir": Path("/root/autodl-tmp/gen-depth-contamination/data/exp_wiki_llama8b"),
        "features_csv": Path("/root/autodl-tmp/gen-depth-contamination/data/exp_wiki_llama8b/features.csv"),
        "needs_rescore": True,
    },
    "mistral7b": {
        "data_dir": Path("/root/autodl-tmp/gen-depth-contamination/data/exp_wiki_mistral7b"),
        "features_csv": Path("/root/autodl-tmp/gen-depth-contamination/data/exp_wiki_mistral7b/features.csv"),
        "needs_rescore": True,
    },
}
...
THETA = 0.60
SEED = 42
OUT_DIR = Path("/root/autodl-tmp/gen-depth-contamination/results/exp_wiki_multi_source")
...
def bootstrap_ci(y, oof_proba, doc_ids, n_boot, rng):
    ...
    for _ in range(n_boot):
        ...
        indices = []
        for d in sampled:
            indices.extend(np.where(doc_ids == d)[0].tolist())
        if len(np.unique(y[indices])) < 2:
            continue
        boot_aucs.append(roc_auc_score(y[indices], oof_proba[indices]))
...
def run_pairwise(X, depths, doc_ids, label, gen_ids=None):
    ...
    for k in range(1, MAX_DEPTH + 1):
        pair = f"{k-1}v{k}"
        mask = np.isin(depths, [k-1, k])
        X_pair = X[mask]
        y_pair = (depths[mask] == k-1).astype(int)
        doc_pair = doc_ids[mask]

        if gen_ids is not None:
            gen_pair = gen_ids[mask]
            X_pair = np.column_stack([X_pair, gen_pair])

        oof_proba, fold_aucs = get_oof_predictions(X_pair, y_pair, doc_pair)
        auc = roc_auc_score(y_pair, oof_proba)
        rng = np.random.RandomState(SEED)
        ci_lower, ci_upper, boot_aucs = bootstrap_ci(y_pair, oof_proba, doc_pair, N_BOOT, rng)

        print(f"  {pair} AUC: {auc:.4f} [{ci_lower:.4f}, {ci_upper:.4f}]  folds={[round(a, 4) for a in fold_aucs]}", flush=True)
...
def run_vocab_only():
    print("\n" + "=" * 60, flush=True)
    print("  PATH A: Wiki Vocab-Only (6 scorer-independent features)", flush=True)
    print("=" * 60, flush=True)
...
def run_unified_scorer():
    ...
    print("\n" + "=" * 60, flush=True)
    print("  PATH B: Wiki Unified Qwen2.5-1.5B Scorer (15D OBD features)", flush=True)
    print("=" * 60, flush=True)
...
def print_summary(vocab_results, unified_results):
    print("\n" + "=" * 60, flush=True)
    print("=== Wiki Multi-Source Results ===", flush=True)
    print("=" * 60, flush=True)
    ...
    print("\n--- Cross-Domain Comparison ---", flush=True)
    print("  C4 multi-source unified K*: 2  (1v2 AUC: 0.7105)", flush=True)
    print("  arXiv multi-source unified K*: 3  (1v2 AUC: 0.8117)", flush=True)
    print("  Wiki single-source K*: 2  (from exp_023_wiki, 1v2 AUC: 0.6193)", flush=True)
    vocab_kstar = vocab_results["no_label"]["kstar"]
    print(f"  Wiki multi-source vocab-only K*: {vocab_kstar}", flush=True)
    if unified_results:
        uni_kstar = unified_results["no_label"]["kstar"]
        print(f"  Wiki multi-source unified K*: {uni_kstar}", flush=True)
...
def main():
    ...
    interim_output = {
        "experiment": "exp_wiki_multi_source",
        "domain": "Wiki",
    ...
    full_output = {
        ...
        "cross_domain_comparison": {
            "c4_unified_kstar": 2,
            "arxiv_unified_kstar": 3,
            "wiki_single_source_kstar": 2,
        },
    }
