...
Cross-model classification retains 80--92\% accuracy with scorer-invariant feature rankings ($\rho > 0.90$), which enables single-classifier deployment across generators.
...
\begin{table}[h]
\centering
\caption{Cross-scorer validation. Alternative scorers (Pythia-1.4B, OLMo-1B) preserve feature ranking ($\rho > 0.90$) and rewrite-mode $\Kempirical = 2$, but reduce continuation-mode $\Kempirical$ from 2 to 1. OLMo arXiv cross-scorer confirms $\Kempirical{=}1$ ($p{=}0.510$, n.s.).}
...
\centering
\small
\caption{Three-way ANOVA (Generator $\times$ Scorer $\times$ Depth-Pair) on per-fold pairwise $\auc$ values. DV = per-fold AUC; $N{=}300$ (12 cells $\times$ 5 folds $\times$ 5 depth-pairs). Scorer is statistically significant but explains modest variance ($\eta^2_p{=}.192$) relative to Depth-Pair ($\eta^2_p{=}.998$), which supports near-scorer-independence of depth discrimination.}
...
Cross-scale validation confirms $\Kempirical = 2$ from 1.5B through 8B: Qwen-7B (1v2 $\auc = 0.6178$ $[0.608, 0.626]$) and LLaMA-3.1-8B (1v2 $\auc = 0.6123$, 0v1 $= 0.9225$) both exceed $\theta$, with progressively lower 0v1 signal consistent with larger models generating more human-like text.
Gemma-2-27B (C4 continuation, Qwen cross-scored) yields $\Kempirical{=}1$ (1v2 $\auc = 0.5895$), consistent with Gemma-2-2B at 5K ($\Kempirical{=}1$, 1v2 $\auc = 0.5981$). The Gemma family produces $\Kempirical{=}1$ at both 2B and 27B; $\Kempirical$ is model-family-dependent rather than scale-dependent.
...
GPT-5.5 achieves 33.2\% accuracy (MAE = 1.10; $N{=}500$) and Claude Opus~4.6 33.1\% (MAE = 1.15; $N{=}499$ after one invalid response) on a balanced test set, comparable to OBD-LightGBM's 38.6\% (95\% CI: $[0.340, 0.422]$; McNemar $p = 0.081$, $0.069$; Table~\ref{tab:s3_llm_zeroshot}).
...
Models with the highest truncation (Pythia, Qwen-7B) still yield $\Kempirical{=}2$ with the same step-function AUC decay pattern as low-truncation models (Table~\ref{tab:kstar_ci}); EOS truncation does not drive the boundary.
...
{\sloppy At all three contamination ratios, fine-tuning on the mixed corpus yields MMLU above the base model ($0.6018$); the clean-data majority dominates the training signal.
...
To control the family-wise error rate across all 10 model configurations, we apply Bonferroni correction: $\alpha_{\text{corr}} = 0.05 / 10 = 0.005$to obtain 99.5\% confidence intervals ($z = 2.807$).
...
OLMo-1B                   & 0v1 & .9861 & .0009 & $[.983,\,.989]$ & \checkmark \\
...
The 2v3 pair remains near chance in both draws ($0.54$, $0.54$); depths $\geq 2$ are indistinguishable regardless of sample.
Qwen-1.5B (continuation, C4) replicates the same pattern ($\Kempirical{=}2$, 1v2 $\auc = 0.6346$).
LLaMA-3.1-8B (continuation, C4) is the sole $\Kempirical{=}2$ configuration where the 10K OOF AUC drops below $\theta$ ($0.5880$; pipeline AUC $= 0.6148$; Bonferroni 99.5\% CI $[0.5776, 0.5978]$); under this stricter criterion $\Kempirical{=}1$.
OLMo-1B (continuation, C4) is the only configuration that yields $\Kempirical{=}1$ at both 5K and 10K, with virtually identical depth profiles (0v1 $\auc = 0.9871$ vs.\ $0.9861$; 1v2 $\auc = 0.4984$ vs.\ $0.5001$); the 1v2 AUC at random level is consistent with Qwen cross-scoring, which places OLMo well below the discrimination boundary.
...
Seven of ten configurations yield identical $\Kempirical$ in 100\% of resamples (CI width~0); LLaMA-8B on C4 is near-perfect at 99.9\%.
The two remaining exceptions, Gemma-2-2B on C4 ($\Kempirical{=}1$ in 92.7\%, $\Kempirical{=}2$ in 7.3\%) and LLaMA-8B on Wikipedia ($\Kempirical{=}1$ in 94.3\%, $\Kempirical{=}2$ in 5.7\%), both sit at the $\Kempirical{=}1/2$ boundary with 95\% CIs of width~1, consistent with their marginal 1v2 AUC values.
...
Together they bound $\Kempirical$ uncertainty from complementary angles: within-sample stability is high (7/10 at 100\%, 8/10 at ${\geq}$99.9\%), and between-sample shifts are limited to boundary configurations (LLaMA-8B, Gemma-2-2B).
...
\centering
\footnotesize
\caption{Bootstrap $\Kempirical$ stability (1{,}000 resamples). Each row reports the fraction of resamples yielding each $\Kempirical$ value and the resulting 95\% CI. Seven of ten configurations are perfectly stable (100\%); LLaMA-8B C4 at 99.9\%; two boundary cases show mild variability (CI width~1).}
...
Table~\ref{tab:gemma27b_transfer} shows 27B$\to$small transfer retains $>$100\% (the 27B model's subtler signal generalizes upward), while ${\leq}$1.5B$\to$27B retains only 76.5\% (excluding LLaMA-8B, which retains 92.1\%).
...
OLMo-1B     & 77.3\% & & & & --- & \\
\midrule
\multicolumn{2}{l}{${\leq}$1.5B$\to$27B avg} & \multicolumn{5}{r}{76.5\%} \\
\bottomrule
\end{tabular}}
\end{table}
...
Binary filtering retains or improves both metrics ($+$0.02\,pp MMLU, $+$0.41\,pp HellaSwag); cross-scale robustness holds without retraining.
